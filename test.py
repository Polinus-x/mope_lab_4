...
while not condition_fisher:
    while not condition_cohren:
        ...
        if not condition_cohren:  #in way cohren is false m+1 and cycle again
            m += 1
    d = sum(student(y, y_average))
    print(f'значущі коефіціентм:{d}')
    yo = []
    for i in range(N):
    ...
    if condition_fisher:
        print('модель адекватна експериментальним даним')
    else:
        print('неадекватно оригіналу')